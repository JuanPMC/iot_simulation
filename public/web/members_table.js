...
function Header(props) {
	...
	for (var groupName of props.groupNames) {
		let members = props.members.get_group_members(groupName)
		var toggle = () => props.toggleGroup(members)
		var on = () => props.onGroup(members)
		var off = () => props.offGroup(members)
		ths.push(
			<th key={groupName}>
				<button className={btnClassAdd}>{groupName}</button>
				<button onClick={toggle} > Toggle </button><button onClick={on} > On </button><button onClick={off} > Off </button>
			</th>
		);
	}
...
function Row(props) {
	...
	var tds = members.get_group_names().map(groupName => {
		...
		return (<td key={groupName}><input type="checkbox" onChange={onChange} checked={checked}/></td>);
	});

	var onAddClick = () => props.onAddMemberToAllGroups(props.memberName);
	var onRmClick = () => props.removeMemberFromAllGroups(props.memberName);
	var state = members.get_state(props.memberName);
	var toggle = () => props.toggleGroup([props.memberName])
	var on = () => props.onGroup([props.memberName])
	var off = () => props.offGroup([props.memberName])

	return (
		<tr>
			<td><button className={btnClassAdd} onClick={onAddClick}>{props.memberName}</button>State: {state.state} Power: {state.power} 
				<button onClick={toggle} >Toggle</button>
				<button onClick={on} >On</button>
				<button onClick={off} >Off</button>
			</td>
			{tds}
			<td><button className={btnClassDel} onClick={onRmClick} >X</button></td>
		</tr>
	);
}
...
function Body(props) {
	var rows = props.members.get_member_names().map(memberName =>
		<Row key={memberName} memberName={memberName} members={props.members}
			...
			removeMemberFromAllGroups={props.removeMemberFromAllGroups}
			toggleGroup={props.toggleGroup}
			onGroup={props.onGroup}
			offGroup={props.offGroup}
			/>);
	...
	return (
		<table className="table table-striped table-bordered">
			<Header 
				...
				get_group_members={props.members.get_group_members()}
